...
const BooksSection = () => {
  ...
  const categories = useMemo(() => {
    ...
    return ['All', ...byPriority];
  }, [books]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    const list = selectedCategory === 'All' ? books : books.filter((b) => b.category === selectedCategory);
    if (!q) return list;
      ...
      <div className="w-full">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-500">The library is currently empty.</p>
          </div>
        )}
      </div>
    </div>
