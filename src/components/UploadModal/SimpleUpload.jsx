  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (result.success && result.url) {
        ...
        setUploadMessage(result.storage === 'local' ? 
      ...
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        ...
        {uploadMessage && (
          <div className={`mt-4 p-3 rounded-lg text-sm ${
            uploadStatus === 'success' ? 'bg-green-100 text-green-700 border border-green-200' :
            uploadStatus === 'error' ? 'bg-red-100 text-red-700 border border-red-200' :
        ...
        {uploading && (
          <div className="mt-4">
            <div className="bg-gray-200 rounded-full h-2">
              <div className="bg-blue-500 h-2 rounded-full animate-pulse transition-all duration-500" style={{width: '70%'}}></div>
            </div>
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          <p className="text-xs text-gray-600">
            <strong>How it works:</strong> Files are processed and stored securely. For images, you'll see a preview immediately. For documents, a placeholder is used for demonstration.
          </p>
        </div>
      </div>
    </div>
