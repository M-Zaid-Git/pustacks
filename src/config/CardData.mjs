const cardData = [
  {
    id: 1,
    image: 'education.webp',
    domain: 'Academics',
    link: '/materials/all?category=Academics',
    urlparams: 'academics',
    background: 'BackgroundImage.webp',
    fetchlink: 'https://64870876beba6297278fb836.mockapi.io/materials',
  },
  {
    id: 2,
    image: 'test.webp',
    domain: 'Competitive Exams',
    link: '/materials/all?category=Competitive%20Exams',
    urlparams: 'competitive-exam',
    background: 'BackgroundImage.webp',
    fetchlink: 'https://64870876beba6297278fb836.mockapi.io/materials',
  },
  {
    id: 3,
    image: 'data-structure.webp',
    domain: 'Computer Science',
    link: '/materials/all?category=Computer%20Science',
    urlparams: 'cp-dsa',
    background: 'BackgroundImage.webp',
    fetchlink: 'https://64870876beba6297278fb836.mockapi.io/materials',
  },
  {
    id: 4,
    image: 'coding.webp',
    domain: 'Engineering',
    link: '/materials/all?category=Engineering',
    urlparams: 'development',
    background: 'BackgroundImage.webp',
    fetchlink: 'https://64870876beba6297278fb836.mockapi.io/materials',
  },
  {
    id: 5,
    image: 'interview.webp',
    domain: 'Business',
    link: '/materials/all?category=Business',
    urlparams: 'interview-placement',
    background: 'BackgroundImage.webp',
    fetchlink: 'https://64870876beba6297278fb836.mockapi.io/materials',
  },
];

export default cardData;
