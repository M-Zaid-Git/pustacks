...
const PasswordValidationModal = () => {
  return (
    <div className="group relative flex items-center justify-center">
      <img src={'/info.png'} className="w-4 h-4 cursor-pointer opacity-70 mb-[0.2rem]" />
...
function SignForm() {
  ...
  return (
    <div
      className=" h-screen flex items-center justify-center border-black pt-[11vh]"
      style={{
    ...
    >
      <div className="bg-white rounded-lg shadow-lg w-96 border-double border-4 border-sky-500">
        <div className="flex justify-between items-center text-center w-full pl-10 pr-10 pt-5 pb-1">
          <h1 className="text-2xl font-bold -m-4 dark:text-[#1A1A1C]" style={{ fontFamily: 'monospace' }}>
            SignUp
          </h1>
        ...
        <form className="px-6 py-4">
          <div
            className={`my-3 text-center text-base bg-red-500 text-white rounded-lg p-1 ${
              errorr ? 'visible' : 'hidden'
            }`}
          >
            {errorr ? errorr : null}
          </div>
          <div
            className={`my-3 text-center text-base bg-red-500 text-white rounded-lg p-1 capitalize ${
              error ? 'visible' : 'hidden'
            }`}
          >
            {error
              ? error.message.replaceAll('Firebase: Error (auth/', '').replaceAll(').', '').replaceAll('-', ' ')
              : null}
          </div>
          <div className="mb-6">
            ...
            <input
              type="text"
              className="w-full border-b-2 border-gray-300 focus:border-blue-500 outline-none dark:text-[#1A1A1C]"
              required
            />
          </div>
          <div className="mb-6">
